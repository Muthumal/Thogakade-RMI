...
//            JasperPrint filledreport = JasperFillManager.fillReport(jasperreport, params, new JRTableModelDataSource(tblItem.getModel()));
        JasperReportViewer reportViewer;
        try {
            reportViewer = new JasperReportViewer(controller.print());
        ...
        if (cmbItemCode.getSelectedIndex() != -1) {
            ...
            if (qty <= qtyOnHand && qty > 0) {
                ...
                for (int i = 0; i < model.getRowCount(); i++) {
                    if (cmbItemCode.getSelectedItem().toString() == null ? model.getValueAt(i, 0).toString() == null : cmbItemCode.getSelectedItem().toString().equals(model.getValueAt(i, 0).toString())) {
                        int qnty = Integer.parseInt(model.getValueAt(i, 3).toString()) + Integer.parseInt(txtQty.getText());
                        double amount = qnty * Double.parseDouble(txtUnitPrice.getText());
                        model.setValueAt(qnty, i, 3);
